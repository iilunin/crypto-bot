...
from API.Entities import APIResult
...
class JWTEndpoint(BotAPIResource):
    ...
    def post(self):
        if not request.is_json:
            return APIResult.ErrorResult(100, "Missing JSON in request"), 400
        ...
        if not username:
            return APIResult.ErrorResult("Missing username parameter"), 400
        if not password:
            return APIResult.ErrorResult("Missing password parameter"), 400

        if username != 'test' or password != 'test':
            return APIResult.ErrorResult("Bad username or password"), 401
